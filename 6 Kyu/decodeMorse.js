...
// feel free to use it. In CoffeeScript, C++, Go, JavaScript, 
// PHP, Python, Ruby and TypeScript, the table can be accessed like this: 
// MORSE_CODE['.--'], in Java it is MorseCode.get('.--'), 
// in C# it is MorseCode.Get('.--'), in Haskell the codes are 
// in a Map String String and can be accessed like this: 
...
// In C#, tests will fail if the solution code throws 
// an exception, please keep that in mind. This is 
// mostly because otherwise the engine would simply 
...
decodeMorse = function(morseCode){
	function trim(s) { 
		// starts with whitespace or ends with whitespace
	    s = s.replace(/(^\s*)|(\s*$)/gi,"");
	    // more than two white space
	    s = s.replace(/[ ]{2,}/gi," "); 
    return s;
}
	...
	var result = [];
  	...
  	for (var i = 0; i < arr.length; i++){
  		if (MORSE_CODE.hasOwnProperty([arr[i]])) {
